...
- **三类观察性非预期变异分级体系**：
  ...
  - `near_homolog`（近同源靶区）：位于计算预测的 guide 同源候选位点及有效 PAM（如 Cas9 `NGG`、Cas12a `TTTV`）附近的小变异。计算预测的邻近关系不代表已确认的真实切割，确认因果关系需结合实验验证。
  - `scattered_snv`（散在点突变，历史沿用名称）：远离计算预测同源靶点的散在点突变及短插入缺失。仅从测序数据无法直接推断具体的生物学诱发机制，可能来源于自发突变、克隆传代漂移等。
