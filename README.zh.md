...
在标准计算节点（8 核心，大肠杆菌 REL606 全基因组高深度再测序真实负载）上的实测指标：

| 评估指标 | 传统工具链 | ProkaDiff | 性能对比 |
| :--- | :--- | :--- | :--- |
| **执行耗时 (Wall-clock)** | 2,666.14 秒 (44 分 26 秒) | **488.65 秒 (8 分 08 秒)** | **提速 5.45 倍 🚀** |
| **峰值内存 (Peak RSS)** | 1.86 GB | 3.09 GB | 内存开销平稳受控 |
| **结构缺失 (DEL) 吻合度** | 基准对照 | 0 bp 绝对匹配 | 100% 召回一致 |
